...
LIVEDATA_FIELD_MAP: dict[str, tuple[str, str]] = {
...
class PVSWebSocket:
    ...
    def is_connected(self) -> bool:
        """Return True if websocket task is running."""
    ...
    def _process_message(self, data: dict) -> None:
        ...
        for ws_field, (var_name, value_type) in LIVEDATA_FIELD_MAP.items():
            if ws_field not in params:
                continue

            new_value = self._convert_value(params[ws_field], value_type)
            old_value = self._live_data.get(var_name)

            if old_value != new_value:
                self._set_live_data_value(var_name, new_value)
    ...
    def _set_live_data_value(self, var_name: str, value: Any) -> None:
        """Set a value on the live data object."""
        if self._live_data is None:
            return
        ...
        attr = attr_map.get(var_name)
        if attr:
            setattr(self._live_data, attr, value)
